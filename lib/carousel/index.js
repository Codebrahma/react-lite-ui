  ...
  render() {
    ...
    return (
      <div className={theme['carousel-container']}>
        ...
        {
          controls &&
          <FaAngleLeft className={cx(theme['left-control'], 'icon-left')} onClick={() => this.setActiveItem(active - 1)} />
        }
        {data.map((item, index) => (
          <CarouselItem
        ...
        {
          controls &&
          <FaAngleRight className={cx(theme['right-control'])} onClick={() => this.setActiveItem(active + 1)} />
        }
      </div>
    );
  }
