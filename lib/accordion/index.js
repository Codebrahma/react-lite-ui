...
class Accordion extends React.Component {
  ...
  toggle = (keyValue, showValue) => {
    ...
    if (this.expandedContentRef[keyValue].clientHeight) {
      ...
      this.expandedContentRef[keyValue].style.height = `${wrapper.clientHeight}px`;
    }
  };

  randerContent = () => {
    const {
      theme, headerKey, contentKey,
    } = this.props;
    const { items } = this.state;
    const iconProps = {
          ...
          && items.map(item => (
            ...
            >
              <div className={classnames(theme.item)}>
                <div
                  className={classnames(theme.heading, { [theme.color]: item.show })}
                  onClick={() => this.toggle(item.key, item.show)}
                >
                  <FaAngleRight
                    {...iconProps}
                    className={classnames(
                      item.show ? theme.open : theme.close,
                    )}
                  />
                ...
                <div
                  ref={(el) => { this.expandedContentRef[item.key] = el; }}
                  className={classnames(
                    { [theme.hideContent]: !item.show },
                    theme.content,
                  )}
                >
                  <p className={classnames(theme.contentText)}>{item[contentKey]}</p>
                </div>
              </div>
            </div>
          ))}
  ...
  render() {
    ...
    return (
      <div className={classnames(theme.accordionBody)}>
        {this.randerContent()}
      </div>
    );
  }
