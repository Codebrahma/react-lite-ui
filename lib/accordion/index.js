...
class Accordion extends React.Component {
  ...
  toggle = (keyValue) => {
    ...
    } else if (updatedShowKey.length === 0) {
      updatedShowKey.push(keyValue);
    } else if (updatedShowKey.includes(keyValue)) {
      updatedShowKey.length = 0;
    } else {
      updatedShowKey.length = 0;
      updatedShowKey.push(keyValue);
    }
    this.setState({ showKey: updatedShowKey });
              ...
              <div className={classnames(theme.item)}>
                ...
                <div
                  className={classnames(
                    theme.heading,
                    { [theme.headerActive]: showKey.includes(item.key) },
                    headerClassName,
                  )}
                ...
                >
                  <FaAngleRight
                    {...iconProps}
                    className={classnames(
                      showKey.includes(item.key) ? theme.open : theme.close,
                    )}
                  />
                ...
                <div
                  ...
                  className={classnames(
                    { [theme.hideContent]: !showKey.includes(item.key) },
                    theme.content,
                  )}
                >
                  <p
                    className={classnames(theme.contentText, contentClassName)}
                  >
                    {item[contentKey]}
                  </p>
                </div>
