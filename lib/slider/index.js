    ...
    if (range) {
      if (left) {
        ...
        if (
          ...
          const width = (e.clientX - findNode(this.sliderBar).offsetLeft)
          ...
          const trackerWidth = (findNode(this.sliderTracker).getBoundingClientRect().width
        ...
        if (
          ...
          const maxValue = (e.clientX - findNode(this.sliderBar).offsetLeft) / sliderBarWidth;
          // Update state and pass values to the onChange callback.
          this.setState(
            {
              valueMax: Math.round(maxValue * max),
            },
            () => {
              const { onChange } = this.props;
              const { valueMin, valueMax } = this.state;
      ...
      if (
        ...
        const width = ((e.clientX - findNode(this.sliderBar).offsetLeft) / sliderBarWidth)
          * 100;
        findNode(this.sliderTracker).style.width = `${width}%`;
        // Update state and pass values to the onChange callback.
        this.setState(
          {
            valueMax: Math.round((width / 100) * max),
          },
          () => {
            const { onChange } = this.props;
            const { valueMin, valueMax } = this.state;
